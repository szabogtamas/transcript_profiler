...
fasta_path = file.path(
  params$data_folder,
  paste(params$gene_symbol, "aa_ensembl_mouse.fa", sep="_")
)
protein_seqs <- aa_seq_from_ensembl(mouse_symbol, mmdb, fasta_path)
protein_seqs
```
...
alignment_fn <- file.path(
  params$data_folder,
  paste(params$gene_symbol,"msa_ensembl_mouse.fa", sep="_")
)
prot_msa <- align_isoforms(protein_seqs, alignment_fn)
...
plot_alignment_models(
  ...
  file.path(
    params$figure_folder,
    paste(params$gene_symbol, "ensembl_proteins_mouse.pdf", sep="_")
  )
)
```
...
plot_trancript_models(fish_symbol, Tx_f, drdb, fig_path)
```
Red regions are annotated as UTR, green as protein coding and blue as nonsense-mediated decay.
...
plot_trancript_models(fly_symbol, Tx_d, dmdb, fig_path)
```
Red regions are annotated as UTR, green as protein coding and blue as nonsense-mediated decay.
...
```{r}
aligned_ranges <- convert_alignment_to_ranges(flyh_alignment)
