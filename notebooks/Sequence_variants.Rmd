...
msa_position_map <- aligned_ranges %>%
  dplyr::rename(ID = transcript) %>%
  mutate(
    Aligned_pos = purrr::map2(start, end, seq)
  ) %>%
  unnest(Aligned_pos) %>%
  arrange(ID, Aligned_pos) %>%
  group_by(ID) %>%
  mutate(
    Original_pos = seq(1, n())
  ) %>%
  ungroup() %>%
  select(ID, Original_pos, Aligned_pos)
...
protein_features %>%
  filter(ID %in% main_prots) %>%
  filter(category == "PTM") %>%
  distinct(ID, begin, end, description) %>%
  arrange(description) %>%
  mutate(
  ...
  left_join(msa_position_map, by=c("ID", begin="Original_pos"))
```
